...
class BaseVLNCETrainer(BaseILTrainer):
    ...
    def _initialize_policy(
        ...
        checkpoint = torch.load('data/checkpoints/map_cma/pred_semantics/iterative_maps/1_ftune/ckpt.7.pth')
        self.policy.load_state_dict(checkpoint, strict=False)

        self.policy.net.instruction_encoder.llava_model = LlavaForConditionalGeneration.from_pretrained(
            "llava-hf/llava-interleave-qwen-0.5b-hf",
            torch_dtype=torch.float16).cuda()

        prompt_embedding_state_dict = torch.load("data/checkpoints/embedding7.pth")
        self.policy.net.instruction_encoder.llava_model.get_input_embeddings().load_state_dict(prompt_embedding_state_dict)

        for param in self.policy.net.instruction_encoder.llava_model.parameters():
            param.requires_grad = False

        prompt_embedding = self.policy.net.instruction_encoder.llava_model.get_input_embeddings()
        ...
        if self.config.MODEL.SEMANTIC_MAP_ENCODER.custom_lr:
            ...
            llava_embeddings_params = list(
                self.policy.net.instruction_encoder.llava_model.get_input_embeddings().parameters())
            fc1_params = list(
                self.policy.net.instruction_encoder.fc1.parameters())
            fc2_params = list(
                self.policy.net.instruction_encoder.fc2.parameters())

            base_params = list(set(regular_params) - set(
                llava_embeddings_params) - set(fc1_params) - set(fc2_params))

            self.optimizer = torch.optim.AdamW([
                {'params': base_params, 'lr': self.config.IL.lr},
                {'params': sem_params, 'lr': self.config.MODEL.SEMANTIC_MAP_ENCODER.lr},
                {'params': llava_embeddings_params, 'lr': 1e-3},
                {'params': fc1_params, 'lr': 1e-3},
                {'params': fc2_params, 'lr': 1e-3},
            ])
            ...
            base_params = list(set(self.policy.parameters()) - set(llava_embeddings_params) - set(fc1_params) - set(fc2_params))
    ...
    def _eval_checkpoint_iterative(
        ...
        observations_o, _, _ = [list(x) for x in zip(*envs.reset())]

        observations = extract_instruction_tokens(
            observations_o, self.config.TASK_CONFIG.TASK.INSTRUCTION_SENSOR_UUID
        )
        ...
        stats_tours = defaultdict(dict)
        dtw_data = defaultdict(list)
