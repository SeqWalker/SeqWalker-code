...
from .encoders.verty import Verification
import json
import numpy as np
import torch
...
from ivlnce_baselines.common.utils import CustomFixedCategorical, batch_to
...
from .encoders.instruction_encoder import encoded_to_string,process_text
...
class MapCMAPolicy(ILPolicy):
    def __init__(
        ...
        self.is_turn_around = [[False,18] for _ in range(25)]
        ...
        self.cnt = 0
        self.is_stop_score = 0.15
        self.mse_loss_fn = nn.MSELoss()
        self.verty = Verification(device=self.device)
    ...
    def act_iterative(
        ...
        if observations['depth'] is not None:
            action_is_forward = action == 1
            if action_is_forward.any():
                depth_mean = torch.mean(observations['depth'],
                                            dim=(1,2,3))
                close_to_zero = depth_mean < 0.05
                if close_to_zero.any():
                    left_mean = torch.mean(
                        observations['depth'][:,:,:128,:],
                        dim = (1,2,3)
                    )
                    right_mean = torch.mean(
                        observations['depth'][:, :, 128:, :],
                        dim=(1, 2, 3)
                    )
                    for i in range(batch_size):
                        if action_is_forward[i] and close_to_zero[i]:
                            if left_mean[i] > right_mean[i]:
                                action[i] = 2
                            elif left_mean[i] < right_mean[i]:
                                action[i] = 3
        ...
        for i in range(batch_size):
            if action[i] == 0:
                ...
                instruction_split_last = instruction_split[i][-1]
                rgb_image = observations["rgb"].permute(0, 3, 1, 2)[i]
                verifier = self.verty
                similar_score = verifier.compute_similarity(instruction_split_last,rgb_image)
                if similar_score < self.is_stop_score:
                    self.is_turn_around[i][0] = True
            if self.is_turn_around[i][0] and self.is_turn_around[i][1]>0:
                action[i] = 3
            else:
                self.is_turn_around[i][0] = False
                self.is_turn_around[i][1] = 18

        if self.is_stop_score >= 0.1 and self.cnt % 10 == 0:
            self.is_stop_score = self.is_stop_score - 0.01
...
class MapCMANet(Net):
    ...
    def forward(
        ...
        text1 = text_embedding.shape
        text2 = dep_embedding.shape
        text3 = map_embedding.shape
